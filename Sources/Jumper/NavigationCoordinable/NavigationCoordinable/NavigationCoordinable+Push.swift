...
extension NavigationCoordinable {
    @discardableResult
    public func push<Output: Coordinable>(
        ...
        navigation.router.push(childContainer, animated: animated)
        return screen
    }
    
    @discardableResult
    public func push<Input, Output: Coordinable>(
...
extension NavigationCoordinable {
    @discardableResult
    public func push(_ route: ScreenRouteKeyPath, animated: Bool = true) -> Self {
        let routeContainer = self[keyPath: route]
        let screen = routeContainer.screen(self)(())
        let childContainer = ChildContainer(
            screenView: screen.view(),
    ...
    @discardableResult
    public func push<Input>(_ route: InputScreenRouteKeyPath<Input>, input: Input, animated: Bool = true) -> Self {
        let routeContainer = self[keyPath: route]
        let screen = routeContainer.screen(self)(input)
        let childContainer = ChildContainer(
            screenView: screen.view(),
